...
def parser(text):
    ...
    Lista_magazine = ["Kaufland","Carrefour", "MegaImage", "Mega Image", "Mega  Image", "Mega   Image",
                      "Mega    Image", "Lidl", "Catena", "HelpNet", "Help Net",
                      "Help  Net", "Help   Net", "Help    Net", "Auchan", "Sensiblu"]

    text = text.upper()

    data_bon = re.search('([0-3][0-9])-([0-1][0-9])-([0-9][0-9][0-9][0-9])', text)
    if (data_bon is None):
        data_bon = re.search('([0-3][0-9]).([0-1][0-9]).([0-9][0-9][0-9][0-9])', text)
    if (data_bon is None):
        data_bon = re.search('([0-3][0-9])/([0-1][0-9])/([0-9][0-9][0-9][0-9])', text)
    ...
    data_fin = ""
    if (data_bon is not None):
        data_fin = data_bon.group()

    magazin_fin = ""
    for i in range(len(Lista_magazine)):
        magazin = Lista_magazine[i].upper()
        if (magazin in text):
            magazin_fin = Lista_magazine[i]

    x_dict = {"magazin" : magazin_fin, "data" : data_fin}
...
imPath = "D:\\python projects\\ProiectImagine\\bon3.jpg"
